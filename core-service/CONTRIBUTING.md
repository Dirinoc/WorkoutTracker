...
2. Проверить, есть ли какие то изменения в ветке master, которых у тебя нет
   - Если есть: подтянуть изменения
   - Если нет: перейти к шагу 3
3. Сделать свои изменения в коде локально
4. Сделать коммит
5. Залить изменения на удаленный репозиторий
6. ОДИН раз создать на github ПР (Pull request)
...
### Проверка на то, нужно ли подтягивать изменения
Если в ветке, в которую ты будешь мержить (то есть master), произошли какие то изменения, тебе ОБЯЗАТЕЛЬНО нужно синкануть свою ветку с той. Иначе будут неприятные мерж конфликты.

Как проверить:
1. `git log --oneline` - эта команда покажет то, какая история коммитов у тебя в develop
2. Зайди на github в ветку, в которую будешь мержить. Нажми на commits.
3. Посмотри, сходится ли история, полученная в шаге 1 с историей, полученной в шаге 2.

Если не сходится:
В локальной ветке develop сделай это
`git pull origin master` - возьмет коммиты из удаленной ветки master, которых нет у тебя в develop, и прибавит их к твоей ветке
если git будет ругаться, то
`git pull --rebase origin master` - то же самое, только прибавит их к ветке немного другим образом
